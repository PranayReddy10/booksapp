...
import com.jntuh.adapter.RateReviewListAdapter;
import com.jntuh.books.databinding.ActivityRateReviewListBinding;
...
public class RateReviewActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        viewRateReviewBinding.toolbarMain.imageArrowBack.setOnClickListener(v -> getOnBackPressedDispatcher().onBackPressed());
        ...
        viewRateReviewBinding.viewRecAll.setOnClickListener(v -> {
            selectStarFilter(0);
            if (rateReviewListAdapter != null && !rateReviewLists.isEmpty())
                rateReviewListAdapter.setList(rateReviewLists);
        });

        viewRateReviewBinding.viewRecStar1.setOnClickListener(v -> {
            selectStarFilter(1);
            if (rateReviewListAdapter != null && !rateReviewLists.isEmpty())
                rateReviewListAdapter.setList(getReviewListByStar("1"));
        });

        viewRateReviewBinding.viewRecStar2.setOnClickListener(v -> {
            selectStarFilter(2);
            if (rateReviewListAdapter != null && !rateReviewLists.isEmpty())
                rateReviewListAdapter.setList(getReviewListByStar("2"));
        });

        viewRateReviewBinding.viewRecStar3.setOnClickListener(v -> {
            selectStarFilter(3);
            if (rateReviewListAdapter != null && !rateReviewLists.isEmpty())
                rateReviewListAdapter.setList(getReviewListByStar("3"));
        });

        viewRateReviewBinding.viewRecStar4.setOnClickListener(v -> {
            selectStarFilter(4);
            if (rateReviewListAdapter != null && !rateReviewLists.isEmpty())
                rateReviewListAdapter.setList(getReviewListByStar("4"));
        });

        viewRateReviewBinding.viewRecStar5.setOnClickListener(v -> {
            selectStarFilter(5);
            if (rateReviewListAdapter != null && !rateReviewLists.isEmpty())
    ...
    private void selectStarFilter(int pos) {
        for (int i = 0; i < viewBg.length; i++) {
            if (i == pos) {
                ...
                tvColor[i].setTextColor(ContextCompat.getColor(RateReviewActivity.this, R.color.orange_star_text));
            } else {
                ...
                tvColor[i].setTextColor(ContextCompat.getColor(RateReviewActivity.this, R.color.orange_star));
            }
        }
